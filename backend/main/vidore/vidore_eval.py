...
"""
Havent tested ViDoRe testsuite yet
"""
...
class MyCustomPipeline(BasePipeline):
    ...
    def __init__(self):
        self.model_name = 'MyTripleHybridEncoder'
    ...
    def index(self, *args, **kwargs):
        # This bridges the synchronous benchmark call to your async logic
        return asyncio.run(self._index(*args, **kwargs))


    async def _index(self, corpus_ids, corpus_images, corpus_texts, dataset_name: str = None):
        # Indexing function to process corpus, should store anything relevant as class attributes

        for i in range(len(corpus_ids)):
            c_id = corpus_ids[i]
            c_image = corpus_images[i]

            with tempfile.NamedTemporaryFile(suffix=".pdf", delete=True, delete_on_close=False) as temp_pdf:

                try:
                    # 2. Save the PIL image directly into the temp file object
                    # Most extraction functions need the file saved in 'PDF' format
                    c_image.save(temp_pdf, format="PDF")
                    
                    # 3. CRITICAL: Flush the data to disk so the extraction function sees it
                    temp_pdf.flush()

                    temp_path = Path(temp_pdf.name)
                    print(f"Processing temporary file at: {temp_path}")
                    ...
                    for chunk in all_chunks:
                        # We transfer the data from the extracted chunk to the ViDoRe-specific one
                        v_chunk = ViDoReChunk(
                            parent_id=c_id,
                            ...
                            # 'type' is handled by the class default, but you can override if needed
                        )
                        formatted_chunks.append(v_chunk)

                    embeddings, _ = await self.model.embed_texts(formatted_chunks)
                    dense_embeddings = [e.vector for e in embeddings]
                    sparse_embeddings = self.sparse.embed_texts(formatted_chunks)
                    late_embeddings = self.late.embed_texts(formatted_chunks)

                    upload_to_qdrant(formatted_chunks,dense_embeddings,sparse_embeddings,late_embeddings)
    ...
    def search(self, *args, **kwargs):
        # This bridges the synchronous benchmark call to your async logic
        return asyncio.run(self._search(*args, **kwargs))


    async def _search(self, query_ids, queries):
        ...
        for q_id in query_ids:
            ...
            dense = await self.model.get_query_vector(query)
            sparse = self.sparse.embed_query(query)
            late = self.late.embed_query(query)

            similar_chunks = get_similar_chunks(dense,sparse,late,limit1=50,limit2=10)

            corpus_scores = defaultdict(list)
            for chunk in similar_chunks:
                corpus_id = chunk['metadata']['corpus_id']
                chunk_score = chunk['score']
                corpus_scores[corpus_id].append(chunk_score)

            final_scores = {c_id : max(scores) for c_id, scores in corpus_scores.items()}
            results[q_id] = final_scores
...
def run_benchmark():
    ...
    # 1. Define the base path (ensure it exists)
    base_path = Path(os.getenv('vidore_results_path'))
    save_path = base_path / task 
    save_path.mkdir(parents=True, exist_ok=True)

    file_path = save_dir / "aggregated_metrics.json"

    with file_path.open("w", encoding="utf-8") as f:
        json.dump(aggregated, f, indent=4, ensure_ascii=True)
